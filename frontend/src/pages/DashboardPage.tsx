        ...
        <Card className="flex flex-col">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-800">{isLoading ? '...' : formatCurrency(totalExpenses)}</h3>
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {tasks.slice(0, 3).map((task) => (
                <div
                  key={task.id}
                  className="flex justify-between items-center p-3 rounded-lg bg-white/40 border border-white/50"
                >
                  <div>
                    <p
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${task.priority === 'high' ? 'bg-rose-100 text-rose-700' : task.priority === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}
                    >
                      {task.priority}
                    </span>
                  </div>
